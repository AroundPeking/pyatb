...
MatrixXcd velocity_solver::cal_pk_1k_base(
    base_data &Base_Data,
    ...
    const int &alpha)
{
    MatrixXcd pk = -IMAG_UNIT * eigenvectors.adjoint() * xr_operation::get_pk(Base_Data, exp_ikR, alpha) * eigenvectors;

    int basis_num = Base_Data.get_basis_num();
    MatrixXcd pk_matrix = MatrixXcd::Zero(basis_num, basis_num);

    for (int row = 0; row < basis_num; row++)
    {
        for (int col = row; col < basis_num; col++)
        {
            pk_matrix(row, col) = pk(row, col);
        }
    }

    pk_matrix.triangularView<Lower>() = (pk_matrix).adjoint();
    ...
    const int &alpha)
{
    ...
    for (int row = 0; row < basis_num; row++)
    {
        for (int col = row; col < basis_num; col++)
        {
            velocity_matrix(row, col) = partial_Hk(row, col) - eigenvalues(row) * partial_Sk(row, col) + IMAG_UNIT * (eigenvalues(row) - eigenvalues(col)) * rk(row, col);
        }
    }
