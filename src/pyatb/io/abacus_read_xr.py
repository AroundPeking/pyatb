...
def abacus_readpR(pR_route, pR_unit, **kwarg):
    if pR_unit == 'eV':
        unit = 1.0
    elif pR_unit == 'Ry':
        unit = Ry_to_eV
        
    with open(pR_route, 'r') as fread:
        while True:
            line = fread.readline().split()
            if line[0] == 'Matrix':
                break
        ...
        pR = [None] * 3
        for direction in range(3):
            pR[direction] = coo_matrix((record_data[direction], (record_row[direction], record_col[direction])), shape=(R_num, triu_size), dtype=complex).tocsc()

    m_pR_x = multiXR('p_x')
    m_pR_y = multiXR('p_y')
    m_pR_z = multiXR('p_z')
    m_pR_x.set_XR(R_num, R_direct_coor, basis_num, pR[0])
    m_pR_y.set_XR(R_num, R_direct_coor, basis_num, pR[1])
    m_pR_z.set_XR(R_num, R_direct_coor, basis_num, pR[2])

    return m_pR_x, m_pR_y, m_pR_z

if __name__ == "__main__":
    pR_route = "/home/ghj/GW/bulk/enable_pR/OUT.ABACUS/data-pR-sparse_SPIN0.csr"
    pR_unit = "Ry"
    m_pR_x, m_pR_y, m_pR_z = abacus_readpR(pR_route, pR_unit)
    print(m_pR_x)